...
rv_objcpy   := riscv-none-elf-objcopy -O verilog --verilog-data-width=1 --no-change-warnings
...
$(hex_dir)/%.hex:$(isa_dir)/%
	...
	@cp $< $(hex_dir)/
	@$(rv_objcpy) -j .text.init --change-section-address .text.init=0 $< $@ >/dev/null
	@$(rv_objcpy) -j .data --change-section-address .data=0 $< $(hex_data) >/dev/null
